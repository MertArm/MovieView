...
func (m *PostgresDBRepo) GetUserByEmail(email string) (*models.User, error) {
	...
	query := `select id, email, first_name,last_name, password, created_at,
...
func (m *PostgresDBRepo) InsertMovie(movie models.Movie) (int, error) {
	...
	err := m.DB.QueryRowContext(ctx, statement, movie.Title, movie.Description,
		movie.ReleaseDate, movie.RunTime, movie.MPAARating, movie.CreatedAt,
		movie.UpdatedAt, movie.Image).Scan(&newID)
...
func (m *PostgresDBRepo) UpdateMovie(movie models.Movie) error {
	...
	statement := `update movies set title = $1, description = $2,
					release_date = $3, runtime = $4, mpaa_rating = $5,
					updated_at = $6, image = $7 where id = $8`

	_, err := m.DB.ExecContext(ctx, statement, movie.Title, movie.Description,
								movie.ReleaseDate, movie.RunTime,
								movie.MPAARating, movie.UpdatedAt, movie.Image,
								movie.ID,)
	if err != nil {
		return err
	}
